;(function () {
  ...
  $.getJSON('/config.json', function (config) {
    ...
    function appendChartLatencyAv (avg) {
      ...
      drawBarChart(CHARTS['latency-avg'], Math.max.apply(null, CHARTS['latency-avg']), '.chart-info#latency-avg .canvas', 1)
    }
    ...
    function appendUpTimeAvg (hosts) {
      ...
      $.each(hosts, function (index, value) {
        ...
        if (Object.keys(hosts).length === nb) {
          avg = avg / nb
          $('.big-info#uptime-avg .data').text(formatPercent(avg) + '%')
          appendChartUpTimeAvg(formatPercent(avg))
        }
      })
    }
    ...
    function appendLoadAvg (hosts) {
      ...
      $.each(hosts, function (index, value) {
        nb++
        avg += value.stats.loadavg && value.stats.loadavg['1m'] ? value.stats.loadavg['1m'] : 0

        // if all callback have been proceed
        if (Object.keys(hosts).length === nb) {
          avg = avg / nb
          $('.big-info#load-avg .data').text(formatPercent(avg) + '%')

          appendChartLoadAvg(formatPercent(avg))
        }
      })
    }
    ...
    function appendChartLoadAvg (avg) {
      ...
      CHARTS['load-avg'].push(avg)

      drawBarChart(CHARTS['load-avg'], Math.max.apply(null, CHARTS['load-avg']), '.chart-info#load-avg .canvas', 1)
    }
