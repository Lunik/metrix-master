    ...
    function appendNodeList (hosts) {
      ...
      $.each(hosts, function (index, value) {
        ...
        var $row = $('<tr>').attr('id', index)
          .append(
            $('<td>').attr('id', 'active').attr('active', value.active).html($('<i>').addClass('fa fa-circle-o-notch')),
            $('<td>').attr('id', 'hostname').text(stats.hostname),
            $('<td>').attr('id', 'type').text(stats.os.name + ' / ' + stats.os.platform + ' / ' + stats.os.release + ' / ' + stats.os.arch),
            $('<td>').attr('id', 'latency').text(stats.latency),
            $('<td>').attr('id', 'cpu').text(stats.cpu.type + ' / ' + stats.cpu.core + 'core / ' + (stats.cpu.speed / 1000) + 'GHz'),
            $('<td>').attr('id', 'loadavg').text(formatPercent(stats.loadavg['1m']) + '% / ' + formatPercent(stats.loadavg['5m']) + '% / ' + formatPercent(stats.loadavg['15m']) + '%'),
            $('<td>').attr('id', 'memory').text(formatData(stats.mem.total - stats.mem.free) + ' / ' + formatData(stats.mem.total) + ' (' + formatPercent((stats.mem.total - stats.mem.free) / stats.mem.total) + '%)'),
            $('<td>').attr('id', 'interfaces').text(stats.interfaces[Object.keys(stats.interfaces)[1]][0].address + ' / '+stats.interfaces[Object.keys(stats.interfaces)[1]][0].mac),
            $('<td>').attr('id', 'speed').text(stats.speed.up + ' / '+stats.speed.down),
            $('<td>').attr('id', 'uptimeAvg').text(formatPercent(stats.upAvg) +'%')
        ).appendTo($table)
      })
