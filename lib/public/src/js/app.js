    ...
    function appendSpeedAvg (hosts) {
      ...
      $.each(hosts, function (index, value) {
        ...
        if (Object.keys(hosts).length === nb) {
          ...
          appendChartSpeedAvg(avg)
        }
      })
    }

    function appendChartSpeedAvg (avg) {
      CHARTS['speed-avg'] = CHARTS['speed-avg'] ? CHARTS['speed-avg'] : {
        'up': getListOf(config.chart.precision - 1, 0),
        'down': getListOf(config.chart.precision - 1, 0)
      }

      CHARTS['speed-avg'].up.push(avg.up)
      CHARTS['speed-avg'].down.push(avg.down)

      drawBarChart(CHARTS['speed-avg'].up, Math.max.apply(null, CHARTS['speed-avg'].up), '.chart-info#speed-avg .canvas#up')
      drawBarChart(CHARTS['speed-avg'].down, Math.max.apply(null, CHARTS['speed-avg'].down), '.chart-info#speed-avg .canvas#down')
    }
    ...
    function drawBarChart (data, max, selector, colorReverse) {
      ...
      $(selector).children().hover(function () {
        ...
        mouseEvent = $(document).mousemove(function (event) {
          ...
          $title.css('left', event.pageX - $title.width() / 2)
        })
      }, function () {
